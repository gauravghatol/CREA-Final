...
async function seedDemoData() {
  const ops = []
  const result = { events: 0, topics: 0, posts: 0, circulars: 0, manuals: 0, courtCases: 0, suggestions: 0, externalLinks: 0, charters: 0 }
  ...
  if (await CharterDemand.countDocuments() === 0) {
    const docs = [
      {
        title: 'Comprehensive Wage Revision',
        summary: 'Ensure parity in compensation with national benchmarks.',
        content: 'Demanding a comprehensive wage revision covering all grades with retrospective effect from January 2025.',
        order: 0,
        published: true,
      },
      {
        title: 'Enhanced Safety Standards',
        summary: 'Invest in modern safety equipment and training.',
        content: 'Introduce advanced monitoring systems, mandatory quarterly safety drills, and upgraded protective gear for all staff.',
        order: 1,
        published: true,
      },
    ]
    ops.push(CharterDemand.insertMany(docs))
    result.charters = docs.length
  }
