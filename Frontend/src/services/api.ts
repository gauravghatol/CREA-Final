...
const toCharter = (dto: CharterDTO): CharterDemand => ({
  id: dto._id,
  title: dto.title,
...
type CharterPatch = Partial<Omit<CharterPayload, 'file'>> & { file?: File }

const buildCharterBody = (input: CharterPayload | CharterPatch): BodyInit => {
  if (input.file) {
    const fd = new FormData()
    if ('title' in input && input.title) fd.append('title', input.title)
    if (input.summary !== undefined) fd.append('summary', input.summary)
    if (input.content !== undefined) fd.append('content', input.content)
    if (input.order !== undefined) fd.append('order', String(input.order))
    if (input.published !== undefined) fd.append('published', String(input.published))
    fd.append('file', input.file)
    return fd
  }
...
export async function getCharterDemands(opts?: { all?: boolean }): Promise<CharterDemand[]> {
  const path = opts?.all ? '/api/charters/all' : '/api/charters'
  const list = await request<CharterDTO[]>(path)
  return list.map(toCharter)
}

export async function createCharterDemand(input: CharterPayload): Promise<CharterDemand> {
  const payload: CharterPayload = { ...input, published: input.published ?? true }
  const body = buildCharterBody(payload)
  const dto = await request<CharterDTO>('/api/charters', { method: 'POST', body })
  return toCharter(dto)
}

export async function updateCharterDemand(id: string, patch: CharterPatch): Promise<CharterDemand> {
  const body = buildCharterBody(patch)
  const dto = await request<CharterDTO>(`/api/charters/${id}`, { method: 'PUT', body })
  return toCharter(dto)
}

export async function deleteCharterDemand(id: string): Promise<{ success: boolean }> {
  await request(`/api/charters/${id}`, { method: 'DELETE' })
  return { success: true }
}
